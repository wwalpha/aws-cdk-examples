...
const self = process.env.AWS_LAMBDA_FUNCTION_NAME as string;

/** Invoke Enter */
// tslint:disable-next-line:variable-name
export const handler = (_event: CloudWatchLogsEvent, _content: Context, callback: Callback) => {
...
const invoke = async () => {
  ...
  funcs.Functions.forEach((func) => {
    ...
    // 除外を除く
    if (self === func.FunctionName) return;
